...
class AuthModule {
  ...
  _getSession () {
    let data = this.ctx.app.$cookies.get('session', { parseJSON: true })
  ...
  _setSession (data, login = false) {
    this.ctx.app.$cookies.set('session', JSON.stringify(data))
    // this.storage.set('session', data)
  ...
  _clearSession () {
    this.ctx.app.$cookies.set('session', undefined, {
      expires: new Date
    })
  ...
  get isLoggedIn () {
    return !!this.ctx.app.$cookies.get('session')
  }
